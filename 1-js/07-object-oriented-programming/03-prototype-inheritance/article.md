...
`[[Prototype]]` 有一个“神奇”的意义。当我们想要从 `object` 中读取一个属性时，并且当前对象中不存在这一属性，JavaScript 会自动从原型中获取它。在编程中，这样的事情称为“原型继承”。许多很酷的语言特性和编程技巧都是基于它的。
...
2. `__proto__` 的值可以是对象，也可以是 `null`。所有其他的类型（例如基本类型）都会被忽略。
