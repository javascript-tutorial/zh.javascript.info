...
我们可以通过使用带有可选 **属性列表** 的花括号 `{…}` 来创建。一个属性就是一个键值对（"key: value"），其中键（`key`）是一个字符串（也叫做属性名），值可以是任何值。
...
通常，检查属性存在性时，使用严格比较 `"=== undefined"` 就够了。但也有这种方式会失败，但 `"in"` 可以胜任的特殊情况。
...
对象是通过引用被赋值或复制的。换句话说，变量存储的不是对象的值“”，而是值的“引用”（内存地址）。所以复制这样的变量或者将其作为函数参数进行传递时，复制的是引用，而不是对象。基于复制的引用（例如添加/删除属性）执行的所有的操作，都是在同一个对象上执行的。
...
他们有着各自特别的特性，我们将在后面学习到。有时候大家会说“数组类型”或“时间类型”，但其实他们并不是自己的类型，而是属于对象类型的一种。他们以不同的方式对对象类型做了一些扩展。
