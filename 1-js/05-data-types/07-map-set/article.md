...
- `map.keys()` —— 遍历并返回一个包含所有键的迭代器，
- `map.values()` —— 遍历并返回一个包含所有值的迭代器，
- `map.entries()` —— 遍历并返回一个包含所有实体 `[key, value]` 的迭代器，`for..of` 在默认情况下使用的就是这个。
...
- `set.keys()` —— 遍历并返回一个包含所有值的迭代器，
- `set.values()` —— 与 `set.keys()` 作用相同，这是为了兼容 `Map`，
- `set.entries()` —— 遍历并返回一个包含所有的实体 `[value, value]` 的迭代器，它的存在也是为了兼容 `Map`。
