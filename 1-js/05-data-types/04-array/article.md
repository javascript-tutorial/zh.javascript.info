...
- `push` 在末端添加一个元素.
- `shift` 从数组的最前端取出一个元素，然后推进队列，这样第二个元素就变成了第一个。
...
对于栈来说，最后放进去的是最先接收的，也叫做LIFO（后进先出）法则。而与序列相对应的叫做FIFO（先进先出）。

JavaScript中的数组既可以用作队列，也可以用作栈。它们允许从前端/末端来添加/删除元素。
...
...但是数组真正特殊的是它们的内部实现。Js引擎尝试把这些元素一个接一个地存储在连续的内存区域，就像本章的插图显示的一样，而且还有一些其它的优化，以使数组运行得非常快。
...
但是Js引擎会发现我们在像使用常规对象一样使用数组，那么数组的这些特殊优化在这种情况下就不再适用了，并且会被关闭，数组的优势也随之消失。
...
请将数组视为作用于*有序数据*的特殊结构，它们为此提供了特殊的方法。数组在JavaScript引擎内部是经过特殊调整的，使得更好的作用于连续的有序数据，所以请以这种方式使用数组。如果你需要任意键值，那很有可能实际上你需要的是常规对象`{}`。
...
2. `for..in` 循环适用于普通对象，不适用于数组，而且会慢10-100倍。当然即使是这样也依然非常快。只有在遇到瓶颈或者一些不相关的场景增速可能会有问题。但是我们仍然应该了解这其中的不同。
...
它很少被使用，因为方括号 `[]` 更短更简洁。而且这种语法还存在一些诡异的特性。
