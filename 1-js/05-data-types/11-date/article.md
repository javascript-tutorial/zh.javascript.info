...
它相当于 `new Date().getTime()`，但它不会创建中间的 `Date` 对象。因此它更快，而且不会对垃圾收集器造成额外的压力。
...
在对一个很耗 CPU 性能的函数进行可靠的基准测试时，我们需要谨慎一点。
...
首先想到的方法可能是连续运行两者很多次，并计算所消耗的时间之差。就这个例子而言，函数过于简短，所以我们必须执行至少 100000 次。
...
看起来使用 `getTime()` 这种方式快得多，原因是它没有类型转化，这样对引擎优化来说更加简单。
...
<http://mrale.ph> 提供了许多关于 V8 引擎的文章。
```
...
Node.js 提供 `microtime` 模块以及其他方法。原则上几乎所有的设备和环境都允许获取更高精度的时间数值，不过需要从 `Date` 以外的对象获取。
