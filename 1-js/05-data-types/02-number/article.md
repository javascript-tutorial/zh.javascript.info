...
2. BigInt 数字，用于表示任意长度的整数。有时会用到它们，因为常规的数字类型无法表示大于 <code>2<sup>53</sup></code> 或小于 <code>-2<sup>53</sup></code> 的数字。由于 BigInt 只会在少数特殊领域中才会用到，因此我们在特殊的章节 <info:bigint> 中对齐进行了介绍。
