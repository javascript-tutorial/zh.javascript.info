...
在 <info:garbage-collection>章节中，我们可以看到，当有一个值被使用（或者可能会被使用）时，JavaScript 引擎将会把该值放到内存中去。
...
例如，如果我们把一个对象放入数组中，只要数组还处于可用状态，那么数组里面的对象依然可访问，即使把该对象的指针置为null
...
// john 已经被存在数组里面,所以它不会被垃圾回收机制回收
// 我们依然可以访问 array[0]
```

类似地，如果我们在一个常规的 `Map` 中使用对象作为属性，只要 `Map` 存在，该它的属性就依然可访问。 它会占据内存空间并且可能不会垃圾回收机制回收
...
*!*
// john 被保存在 map 中
// 我们可以通过 map.keys() 来访问它
*/!*
```
`WeakMap` 在这方面就有着很大的不同，它不会阻止垃圾回收机制回收对象的属性
