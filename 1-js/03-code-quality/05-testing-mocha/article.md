...
为了确保函数可以很好地工作，我们来使用更多值测试它吧。除了手动地编写 `it` 代码块，我们可以使用 `for` 循环来生成他们：
...
我们继续添加更多的测试。但在此之前，我们需要注意到辅助函数 `makeTest` 和 `for` 应该被组合到一起。我们在其他代码中不需要 `makeTest`，只有在 `for` 循环中需要它：他们共同的任务就是检查 `pow` 是怎么提高给定的数值的。
...
将来，我们可以在顶级域中添加更多的 `it` 和 `describe` 以及它们自身的辅助函数，它们不会看到 `makeTest`。
