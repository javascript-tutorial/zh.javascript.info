...
大多数情况下，脚本需要处理信息。如果是个网上商店，就会有商品和购物车；如果是个聊天网站，就会有用户、消息等等。
...
如果命名包括多个单词，通常采用 [camelCase](https://en.wikipedia.org/wiki/CamelCase) 命名规则。也就是，单词一个接一个，每个单词以大写字母开头：`myVeryLongName`。
