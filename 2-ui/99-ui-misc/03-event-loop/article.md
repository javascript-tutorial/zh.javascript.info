...
当我们浏览一个网页时就是上述这种形式。JavaScript 引擎大多数时候不执行任何操作，仅在脚本/处理程序/事件激活时运行。

任务示例：

- 当外部脚本 `<script src="...">` 加载完成时，任务就是执行它。
- 当用户移动鼠标时，任务就是派生出 `mousemove` 事件和执行处理程序。
- 当定时器 `setTimeout` 时间到期时，任务就是运行其回调。
- ……诸如此类。
...
两个细节：
1. 引擎执行任务时永远不会进行渲染。如果任务执行需要很长一段时间也没关系。仅在任务完成后才会绘制对 DOM 的更改。
...
我们可以通过将大任务拆分成多个小任务来避免这个问题。高亮显示前 100 行，然后使用 `setTimeout`（延时参数为 0）来安排后 100 行的高亮显示，依此类推。
...
单次运行 `count` 会完成工作 `(*)` 的一部分，，然后根据需要重新安排自身的执行 `(**)`：

1. 首先运行计数：`i=1...1000000`。
2. 然后运行计数：`i=1000001..2000000`。
3. ……以此类推。
...
需要关注的是两者变体 —— 有和没有用 `setTimeout` 拆分任务 —— 在执行速度上是相当的。在执行计数的总耗时是没有多少差异的。
...
微任务仅仅由我们的代码产生。它们通常由 promises 生成：对于 `.then/catch/finally` 的处理程序变成了一个微任务。微任务通常"隐藏在" `await` 下，因为它也是另一种处理 promise 的形式。
...
还是跟前面的类似的“计数型进度展示条”的例子，不同的是用 `queueMicrotask` 来代替 `setTimeout`。你可以看到它在最后才渲染。就像写的是同步代码：
...
也被用在事件处理程序中来定时执行一个行为，在当前事件被完全处理（冒泡结束）之后。

计划一个新的**微任务**：
- 使用 `queueMicrotask(f)`。
- promise 的处理程序也是进入到微任务队列。
