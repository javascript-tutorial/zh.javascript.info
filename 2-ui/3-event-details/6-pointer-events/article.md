...
- 开始的开始，浏览器只用处理鼠标事件。
    ...
    后来，触屏设备出现了。为了让以前的代码正常运行，触屏事件继续沿用了以前的鼠标事件。比如，轻触屏幕就会触发 `mousedown` 事件。但由于触摸设备往往支持多点触控等特性，而鼠标事件无法处理这些更加强大的特性，就显得心有余而力不足了。
...
- 为了解决这些问题，人们引入了全新的「指针事件」。它提供了一套处理来自各种输入设备的统一的事件。

目前，各大主流浏览器已经支持了 [Pointer Events Level 2](https://www.w3.org/TR/pointerevents2/) 标准，[Pointer Events Level 3](https://w3c.github.io/pointerevents/) 也已经在路上了。所以除非你专门为 IE 10 或 Safari 12 以下版本的浏览器构建代码，否则无需继续使用鼠标事件或触摸事件，可以安全地使用指针事件。

话虽如此，指针事件仍然有一些重要的奇怪特性，你应当对它们有所了解来避免一些意料之外的错误。我们会在这篇文章中一一介绍。
...
替换之后，程序对触屏设备的支持会”魔法般“地提升。但你可能需要在 CSS 中添加 `touch-action: none` 一行。我们会在 `pointercancel` 一节描述这里面的细节。
```

## 指针事件属性

指针事件具备和鼠标事件完全相同的属性，包括 `clientX/Y`，`target` 等等，以及一些额外的属性：

- `pointerId` - 用来标识触发当前指针事件的独特 ID
    
    你可以用它来处理多点触控（比如使用触控笔或多点触控屏）的情况。
- `pointerType` - 指针的设备类型，为一个字符串。可以是："mouse", "pen" 或 "touch"。

    我们可以使用这个属性来针对不同种类的指针输入执行不同的操作。
- `isPrimary` - 当指针为首要指针（多点触控时按下的第一根手指）时为 `true`。

对于一些能够测量接触面积和点按压力的指针（如压感触屏），这些额外属性会比较实用：

- `width` - 触摸接触面积的宽度。对于鼠标和其他不支持的设备，这个值总是 `1`。
- `height` - 触摸接触面积的长度。对于鼠标和其他不支持的设备，这个值总是 `1`。
...
这些属性在不同设备上的支持相对不完善，所以使用得比较少。在需要时你可以查看指针事件的 [描述文档](https://w3c.github.io/pointerevents/#pointerevent-interface)。

## 多点触控

多点触控（用户同时在手机或平板上点击若干个位置）是鼠标事件完全不支持的事件之一。

指针事件通过 `pointerId` 和 `isPrimary` 属性让我们能够处理多点触控事件。
...
2. 第二次和后续的更多触摸:
    - `pointerdown` 事件触发，`isPrimary=false` 并且每一次触摸都被指派了不同的 `pointerId`。

请注意：`pointerId` 不是分配给整个设备的，而是分配给每一次触摸的。如果 5 根手指同时触摸屏幕，我们会得到 5 个 `pointerdown` 事件和相应的坐标以及 5 个不同的 `pointerId`。

和第一次触摸相关联的事件总有 `isPrimary=true`.

利用 `pointerId`，我们可以追踪多个并列的触摸事件。当用户移动或抬起某跟手指时。我们会得到和 `pointerdown` 事件具有相同 `pointerId` 的 `pointermove` 或 `pointerup` 事件。
...
请注意：你需要使用一个多点触控设备（如平板或手机）来看到区别。对于使用鼠标的单点触控设备，所有的事件都会具有相同的 `pointerId` 和 `isPrimary=true` 属性。
```
...
**言而总之：由于我们无需在整个 `document` 上添加/移除处理程序，代码就变得整洁多了。这就是指针捕捉的主要意义。**
...
目前，指针事件已经被各大主流浏览器支持，所以如果不需要支持 IE10 和 Safari 12 以下的版本，我们可以放心地切换到它们。不过即便是针对这些浏览器，也可以使用 polyfill 来让它们支持指针事件。
