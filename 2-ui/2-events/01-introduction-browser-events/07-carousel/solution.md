...
为了是列表水平显示，我们需要为 `<li>` 应用正确的 CSS 属性，比如 `display: inline-block`。

对于 `<img>` 来说，我们应该调整 `display`，因为默认情况下它是 `inline`。在 `inline` 元素下为 "letter tails" 保留了额外的空间，因此我们可以使用 `display:block` 来移动它。
