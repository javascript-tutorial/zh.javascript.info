...
可能会有多个名称相同的元素，这种情况经常在处理单选按钮时出现。
...
这些导航属性并不依赖于标签的结构。所有的元素无论它们在表单中嵌套的有多么深，都可以通过 `form.elements` 获取到。
...
<script>
  ...
  // form.elements 更新 name:
  alert(form.elements.login); // 未定义
...
通常来说，我们可以使用 `input.value` 或者 `input.checked` 来作为复选框的值访问。
...
`form.elements`  
：表单元素以 `form.elements[name/index]` 的形式提供，或者也可以使用 `form[name/index]`。`elements` 属性也适用于 `<fieldset>`。
