...
3. 即使用户离开页面，弹窗依然可以存在。例如，一个咨询建议的用户访问了一个新的 “Super-Cooler” 活动页。用户在主窗口中进入那里而不离开聊天。
...
还有一些针对某些浏览器不太受支持的特定功能，通常不使用这些功能。详情请看 <a href="https://developer.mozilla.org/en/DOM/window.open">window.open in MDN</a>。
...
对于具有来自其他站点的 URL 的窗口，我么可以通过指定 `newWindow.location=...` 来改变地址，但是我们无法读取地址或者访问内容。这是为了用户安全，这样一来恶意网站就无法打开 `http://gmail.com` 并获取数据。我们将在稍后进一步讨论。
...
理论上, 有 `window.focus()` 和 `window.blur()` 方法来对一个窗口聚焦和失焦。此外还有 `focus/blur` 事件，可以聚焦窗口并捕捉访问者切换到其他地方的时机。

在过去恶意网站滥用这些。例如, 看这段代码:
