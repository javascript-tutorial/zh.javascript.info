...
下面这些是不允许的，必须要编码才可以，例如 非拉丁字符，空格 —— 以它们的 UTF-8 代码代替，前缀为 `%`，例如 `20%`（基于历史原因，空格可以通过 `+` 编码，因此在 URL 中也可以这样做）。
...
正如所见，`:`，`?`，`=`，`&`，`#` 这类字符在 URL 中是允许的。而其他的，包括非拉丁字符和空格，则必须被编码。
...
这就是 `encodeURIComponent` 所做的事情。它编码与 `encodeURI` 以及许多其他字符相同的字符，以使得结果值在任何 URL 组件中都可以安全使用。
...
但是我们应该在搜索参数中编码 `&`，否则，我们得到的是 `q=Rock&Roll` —— 实际上是 `q=Rock` 加上一些模糊的参数 `Roll`。其结果和于其不同。
