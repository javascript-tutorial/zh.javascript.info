...
为了更好的说明，假设有一个任务。我们需要找出引号：要么是单引号 `subject:'...'`，要么是双引号 `subject:"..."` —— 这两种类型都要找出来。
...
如你所见，这个模式发现了左双引号 `match:"`，然后文本全部匹配直到另一个引号 `match:'` 匹配，结束本次匹配。
...
- 组内标记 `?:` 则无法引用到该组。正则表达式引擎不会记忆用 `(?:...)` 标记的捕获组。
